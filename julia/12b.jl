...
function greedy_search(grid, visited, i)
    ...
    for direction ∈ directions
        ...
        if checkbounds(Bool, grid, pos_new) && grid[pos_new] == grid[i]
            ...
            left = i + CartesianIndex(0, -1)
            top = i + CartesianIndex(-1, 0)
            if !(checkbounds(Bool, grid, left) && grid[left] == grid[i] && !(checkbounds(Bool, grid, left + direction) && grid[left + direction] == grid[i]) ||
                checkbounds(Bool, grid, top) && grid[top] == grid[i] && !(checkbounds(Bool, grid, top + direction) && grid[top + direction] == grid[top]))
                perimeter += 1
            end
        end
    end
